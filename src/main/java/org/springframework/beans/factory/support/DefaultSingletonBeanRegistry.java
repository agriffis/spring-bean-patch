 ...
 * {@link org.springframework.beans.factory.config.SingletonBeanRegistry}. Allows for registering
 * singleton instances that should be shared for all callers of the registry, to be obtained via
 * bean name.
 * 
 * <p>
 * Also supports registration of {@link org.springframework.beans.factory.DisposableBean} instances,
 * (which might or might not correspond to registered singletons), to be destroyed on shutdown of
 * the registry. Dependencies between beans can be registered to enforce an appropriate shutdown
 * order.
 * 
 * <p>
 * This class mainly serves as base class for {@link org.springframework.beans.factory.BeanFactory}
 * implementations, factoring out the common management of singleton bean instances. Note that the
 * {@link org.springframework.beans.factory.config.ConfigurableBeanFactory} interface extends the
 * {@link SingletonBeanRegistry} interface.
 * 
 * <p>
 * Note that this class assumes neither a bean definition concept nor a specific creation process
 * for bean instances, in contrast to {@link AbstractBeanFactory} and
 * {@link DefaultListableBeanFactory} (which inherit from it). Can alternatively also be used as a
 * nested helper to delegate to.
 * 
...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/**
	 * Names of beans currently excluded from in creation checks (using a ConcurrentHashMap as a
	 * Set)
	 */
	private final ConcurrentMap<String, Boolean> inCreationCheckExclusions = new ConcurrentHashMap<String, Boolean>(16,
				0.75f, 32);
	...
	private final ConcurrentMap<String, Set<String>> containedBeanMap = new ConcurrentHashMap<String, Set<String>>(16,
				0.75f, 32);

	/** Map between dependent bean names: bean name --> Set of dependent bean names */
	private final ConcurrentMap<String, Set<String>> dependentBeanMap = new ConcurrentHashMap<String, Set<String>>(64,
				0.75f, 32);

	/**
	 * Map between depending bean names: bean name --> Set of bean names for the bean's dependencies
	 */
	private final ConcurrentMap<String, Set<String>> dependenciesForBeanMap =
		new ConcurrentHashMap<String, Set<String>>(64, 0.75f, 32);
	...
	protected void addSingleton(String beanName, Object singletonObject) {
		synchronized (this.singletonObjects) {
			this.singletonObjects.put(beanName, singletonObject != null ? singletonObject
				: DefaultSingletonBeanRegistry.NULL_OBJECT);
	 ...
	 * <p>
	 * To be called for eager registration of singletons, e.g. to be able to resolve circular
	 * references.
	 * 
	 * @param beanName
	 *            the name of the bean
	 ...
	 * <p>
	 * Checks already instantiated singletons and also allows for an early reference to a currently
	 * created singleton (resolving a circular reference).
	 * 
	 * @param beanName
	 *            the name of the bean to look for
	 * @param allowEarlyReference
	 *            whether early references should be created or not
	 ...
	 *            the name of the bean
	 * @param singletonFactory
	 *            the ObjectFactory to lazily create the singleton with, if necessary
	 ...
	 * <p>
	 * Disposable beans usually correspond to registered singletons, matching the bean name but
	 ...
	 * <p>
	 * Also registers the containing bean as dependent on the contained bean in terms of destruction
	 * order.
	 * 
	 * @param containedBeanName
	 *            the name of the contained (inner) bean
	 * @param containingBeanName
	 *            the name of the containing (outer) bean
	 * @see #registerDependentBean
	 */
	public void registerContainedBean(String containedBeanName, String containingBeanName) {
		getMappedSet(containedBeanMap, containingBeanName).add(containedBeanName);
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		Set<String> mappedSet = getMappedSet(dependentBeanMap, canonicalName);
		mappedSet.add(dependentBeanName);
	...
	private Set<String> getMappedSet(ConcurrentMap<String, Set<String>> map, String name) {
		...
		map.putIfAbsent(name, Collections.synchronizedSet(new LinkedHashSet<String>(8)));
		return map.get(name);
	}
	 ...
	 * <p>
	 * Subclasses should synchronize on the given Object if they perform any sort of extended
